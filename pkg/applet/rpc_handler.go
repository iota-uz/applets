...
// requirePermissions checks that the current user has all required permissions.
// Uses the host services to extract the user from context.
func (c *AppletController) requirePermissions(ctx context.Context, required []string) error {
	u, err := c.builder.host.ExtractUser(ctx)
	if err != nil {
		return fmt.Errorf("applet.requirePermissions: %w", err)
	}
	if u == nil {
		return fmt.Errorf("applet.requirePermissions: no user: %w", ErrPermissionDenied)
	}
	for _, need := range required {
		...
		if !u.HasPermission(need) {
			return fmt.Errorf("applet.requirePermissions: missing permission %q: %w", need, ErrPermissionDenied)
		}
	}
	return nil
}
