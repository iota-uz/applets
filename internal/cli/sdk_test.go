...
func TestResolveSDKRoot_UsesFlagWhenProvided(t *testing.T) {
	tmp := t.TempDir()
	...
	got, autoDetected, err := resolveSDKRoot(root, flagSDK)
...
func TestResolveSDKRoot_AutoDiscoveryFindsSiblingApplets(t *testing.T) {
	...
	got, autoDetected, err := resolveSDKRoot(root, "")
	require.NoError(t, err)
...
func TestResolveSDKRoot_AutoDiscoveryFindsNestedAppletsDir(t *testing.T) {
	tmp := t.TempDir()

	root := filepath.Join(tmp, "workspace", "eai")
	require.NoError(t, os.MkdirAll(root, 0o755))
	writePackageJSON(t, root, `{"name":"@eai/project"}`)

	nestedSDK := filepath.Join(root, "applets")
	require.NoError(t, os.MkdirAll(nestedSDK, 0o755))
	writePackageJSON(t, nestedSDK, `{"name":"@iota-uz/sdk"}`)

	got, autoDetected, err := resolveSDKRoot(root, "")
	require.NoError(t, err)

	absNestedSDK, err := filepath.Abs(nestedSDK)
	require.NoError(t, err)
	assert.Equal(t, absNestedSDK, got)
	assert.True(t, autoDetected)
}
