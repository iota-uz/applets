...
func (m *managedProcess) runCritical(ctx context.Context, exitCh chan<- string) {
	for {
		...
		case <-m.restartCh:
			...
			continue
		default:
			// Any non-restarted exit of a critical process is treated as fatal and triggers full shutdown.
			exitCh <- m.spec.Name
			return
		}
	}
...
func startProcess(ctx context.Context, spec ProcessSpec, padLen int, shutdownTimeout time.Duration) (*exec.Cmd, error) {
	...
	if errOut != nil {
		log.Printf("warning: stdout pipe for %s: %v", spec.Name, errOut)
	}
	stderr, errErr := cmd.StderrPipe()
	if errErr != nil {
		log.Printf("warning: stderr pipe for %s: %v", spec.Name, errErr)
	}
