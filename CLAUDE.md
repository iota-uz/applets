...
This repository is the applet framework and the private source workspace for the former applet UI. It is **library-only**: no runnable applets (e.g. BiChat) live here.
...
- **Private UI workspace**: root `package.json` and `ui/`. It is not published. Public `@iota-uz/sdk` ownership and releases live in `iota-uz/iota-sdk`.
...
- **NPM package local development**: Use **`applet dev`** from the consumer repo. In conventional layouts it auto-detects `./applets` (or a sibling `../applets`) and wires consumers to a managed local SDK package. For non-conventional layouts, use `applet dev --sdk-root /path/to/applets`.
...
just watch-npm  # Build npm package JS in watch mode
```

## Workflows

- **CI** (`.github/workflows/ci.yml`): on push/PR — Go test, `pnpm install --frozen-lockfile`, `pnpm run build`, optionally lint.
- This repository has no npm publish workflow. Public SDK releases are owned by `iota-uz/iota-sdk`.
